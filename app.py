...
# ------------------------------------------------------------------------------
    ...
    st.caption("This screen will check the **current QR version** every 30 seconds. If HR manually regenerates QR, it will update automatically. Daily midnight (IST) rotation is handled by the backend; this screen will naturally show the new token on the next poll.")

    # Poll version endpoint every 30 seconds via Streamlit's autorefresh
    _ = st.cache_data(ttl=1)(lambda: None)()  # no-op (just here to reflect memo usage)
    st_autorefresh = st.rerun()
  # fallback if needed

    # Use Streamlit built-in autorefresh utility
    count = st.experimental_get_query_params().get("_autorefresh", [0])[0]
    try:
        count = int(count)
    except Exception:
        count = 0
    ...
    with col1:
        if res and res.status_code == 200 and ok:
            ...
            # If token changed (regenerated by HR or rotated), redraw QR
            changed = token != st.session_state.get("qr_last_token")
            if changed:
                st.session_state["qr_last_token"] = token

            # Build deep link for employee (frontend base URL + query params)
            params = {"company_id": company_id, "qr_token": token}
            ...
            st.markdown(f"🔗 **Employee Link:** {deep_link}")
    ...
    with col2:
        # Trigger a re-render every 30 seconds
        st.info("Auto-checking every 30 seconds…")
        # Streamlit offers st.experimental_rerun or st_autorefresh widget; emulate with JS meta-refresh approach:
        st.markdown(
            """
